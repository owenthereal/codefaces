 ...
 *******************************************************************************/
package org.codefaces.ui.lookandfeel.spica.managers;

import java.util.ArrayList;
import java.util.List;

import org.codefaces.ui.lookandfeel.spica.Style;
...
public class MenuBarManager extends MenuManager {
	...
	public void fill(final Composite parent) {
		...
		toolbar.setData(WidgetUtil.CUSTOM_VARIANT, Style.MENUBAR_MAIN);
		update(false, false);
	}
	...
	private void makeEntry(final IContributionItem item) {
		...
		if (tempItem != null && tempItem instanceof MenuManager) {
			...
			toolItem.setData(WidgetUtil.CUSTOM_VARIANT, Style.MENUBAR_MAIN);
	...
	private void createMenu(final MenuManager manager, final ToolItem toolItem) {
		...
		menu.setData(WidgetUtil.CUSTOM_VARIANT, Style.MENUBAR_MAIN);
	...
	private void styleMenuItems(final Menu menu) {
		...
		if (items != null && items.length > 0) {
			for (int i = 0; i < items.length; i++) {
				items[i].setData(WidgetUtil.CUSTOM_VARIANT, Style.MENUBAR_MAIN);
				Menu subMenu = items[i].getMenu();
				if (subMenu != null) {
					subMenu.setData(WidgetUtil.CUSTOM_VARIANT,
							Style.MENUBAR_MAIN);
