...
public class SVNFetchFileInfoOperationHandlerTest {
	...
	public void fetchFileInfoShouldGetFileContentForFileInRoot() {
		...
		para.addParameter(SCMOperationHandler.PARA_REPO_FILE, mockFile);
		RepoFileInfo info = handler.execute(connector, para);

		assertTrue(info.getContent().startsWith(
				TEST_PREFIX_FOR_TESTFILE_IN_TRUNK));
	}

	@Test
	public void fetchFileInfoShouldGetFileContentForFileInFolder() {
		RepoFile mockFile = createMockFile(TEST_URL_IN_NORMAL_STRUCTION,
				TEST_USERNAME, TEST_PASSWORD, TEST_FILEPATH_IN_SOME_FOLDER);
		SCMOperationParameters para = SCMOperationParameters.newInstance();
		para.addParameter(SCMOperationHandler.PARA_REPO_FILE, mockFile);
		RepoFileInfo info = handler.execute(connector, para);

		assertTrue(info.getContent().startsWith(
				TEST_PREFIX_FOR_TESTFILE_IN_SOME_FOLDER));
	}
