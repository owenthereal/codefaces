...
/**
 * 
 * Ref: <Effective Java> Item 67 Remark: I don't think we need to use Executor
 * at this content
 * 
 * @author kklo
 */
public class WorkSpace {
	...
	public void update(RepoBranch newRepoBranch) {
		...
		synchronized (lock) {
			...
			if (!newRepoBranch.getRepo().getName()
					.equals(workingRepo.getName())) {
	...
	/**
	 * Change the current working repository and branch. Please notice that any
	 * changes in repository must accompanied by a branch changes
	 * 
	 * @param newRepo
	 *            a new repository
	 * @param newRepoBranch
	...
	public void removeWorkSpaceChangeEventListener(
			WorkSpaceChangeEventListener w) {
